...
def test_execution_disabled_and_paper_behavior_unchanged():
    ...
    # Paper trader portfolio initial state
...
def test_event_local_flow_does_not_read_global_cvd():
    ...
    # Ensure engine accumulator starts from 0 at event_time_ms
...
def test_recorder_failure_does_not_change_scorer_output():
    engine = OrderFlowEngine()
    if engine.liquidity_recorder:
        engine.liquidity_recorder.telemetry.status = "DEGRADED"
        engine.liquidity_recorder.telemetry.failure_count += 1

    alerts_history = [{"symbol": "btcusdt", "type": "LARGE_TRADE", "timestamp": 10.0}]
    score, details, status, ob_imbalance, depth_age = engine.scorer.evaluate_sweep(
        "btcusdt", "BULLISH", 100.0, alerts_history, 10.0
    )
    assert score is not None


def test_recorder_failure_does_not_change_classification():
    engine = OrderFlowEngine()
    if engine.liquidity_recorder:
        engine.liquidity_recorder.telemetry.status = "DEGRADED"

    # Engine classification operates unaffected
    res = engine.liquidity_engine.symbol_runtime("BTCUSDT")
